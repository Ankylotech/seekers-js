class Seeker extends GameObject {
    constructor(player = player.p5) {
        super();
        this.player = player;
        this.color = this.player.color;
        this.pos = createVector(Math.random() * width / 2 + width / 2 * this.player.side,
            Math.random() * height);
        this.maxSpeed = 1.25;
    ...
    influence(goal) {
        if (this.magnetStatus !== 0 && !this.disabled) {
            ...
            accVec = this.boundBy(accVec, -width, width, -height, height, 2)
    ...
    setAcc() {
        ...
        this.acc = this.boundBy(this.acc, -width, width, -height, height, 2)
